...
model.summary()
history = model.fit(X_train, Y_train, batch_size=X_train.shape[0], epochs=1500, validation_data=(X_val, Y_val))
plot_history(history, "baseline")

model2 = create_nn_model()
history = model2.fit(X_train, Y_train, batch_size=X_train.shape[0], epochs=80, validation_data=(X_val, Y_val))
